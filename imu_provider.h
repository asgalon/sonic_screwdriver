...
protected:
  ...
  void EstimateGravityDirection(float* gravity) const {
    ...
    *done_just_triggered = false;
    ...
    for (int i = 0; i < new_samples; ++i) {
      ...
      *done_just_triggered = old_state != eDone && *stroke_state == eDone;
      ...
      const float mean[3] = {
        x_total / stroke_length,
        y_total / stroke_length,
        z_total / stroke_length
      };
      ...
      *stroke_transmit_length = stroke_length / stroke_transmit_stride;
      ...
      for (int j = 0; j < *stroke_transmit_length; ++j) {
        ...
        for (int i = 0; i < 3; i++) {
          n[i] = (orientation_entry[i] - mean[i]) / range;
        }
      
        const float x_axis = xaxisz * n[2] + xaxisy * n[1];
        const float y_axis = yaxisz * n[2] + yaxisy * n[1];
