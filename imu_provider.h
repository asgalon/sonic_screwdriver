...
auto stroke_points = reinterpret_cast<int8_t*>(stroke_struct_buffer + sizeof(int32_t) * 2);
auto stroke_transmit_length = reinterpret_cast<int32_t*>(stroke_struct_buffer + sizeof(int32_t));
auto stroke_state = reinterpret_cast<int32_t*>(stroke_struct_buffer);
...
public:
  ...
  void UpdateVelocity(const int new_samples) {
  ...
  void UpdateStroke(const int new_samples) {
